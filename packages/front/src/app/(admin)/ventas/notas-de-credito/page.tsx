...
export default function NotasCreditoPage() {
  ...
  const { data: notas, isLoading } = useGetVentasRawQuery({
    filter: JSON.stringify({ tipoOperacion: 'nota_credito' }),
  ...
  return (
    <div className="p-6 space-y-6">
      <PageTitle title="Notas de Crédito">
        <Button size="sm" onClick={() => router.push('/ventas/nueva')}>
          <Plus className="h-4 w-4 mr-2" /> Nueva NC
        </Button>
      </PageTitle>
            ...
            <div key={v.id} className="flex items-center justify-between px-4 py-3">
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-medium text-sm">NC #{v.id}</span>
                  <Badge
                    variant="outline"
              ...
              <div className="flex items-center gap-3">
                <span className="font-semibold text-red-600">-{formatMoney(parseFloat(v.total ?? '0'))}</span>
